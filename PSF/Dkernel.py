...
if __name__ == "__main__":
    ...
    if arm in ("Y", "J", "H"):
        ...
        detsize = (4096, 4096)
        sampling = 2.1
        skipfib = [3,4,36,37,39,40,72,73]
    elif arm in ("U", "B", "V", "R", "IZ"):
        n_fibers = 66
        detsize = (9216,9232)
        sampling = 4.0
        skipfib = [32,35]
    ...
    if kerpara_given:
        try:
            ...
            print('Error: <kerpara> must be four comma-separated values, e.g., 5,5,3.2,left')
            sys.exit(1)
        # convert from FWHM to sigma and scale by sampling
        kerpara[2] = float(kerpara[2]) * sampling/2.35
    ...
    if kerpara_given:
        try:
            import matplotlib.pyplot as plt
            # Use kernel from the first processed fiber as an example
            kernel_example = custom_gaussian_kernel(*kerpara)
            plt.imshow(kernel_example, interpolation='nearest')
            plt.colorbar()
            plt.title("Modified Gaussian Kernel")
            plt.show()
        except ImportError:
            pass
