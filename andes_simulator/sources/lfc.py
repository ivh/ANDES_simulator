...
class LFCSource:
    ...
    def _generate_lfc_wavelengths(self) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        """
        wl_min, wl_max = BAND_WAVELENGTH_RANGES[self.band]
    ...
    def _create_lfc_source(self) -> CSVSource:
        """
        ...
        """
        wavelengths, fluxes = self._generate_lfc_wavelengths()

        # write to temp file
        lfc_dir = self.project_root / 'SED' / '.lfc_temp'
        lfc_dir.mkdir(exist_ok=True)

        lfc_filename = f'LFC_{self.band}_{self.lines_per_order}lpo_{self.flux_per_line:.0e}.csv'
