...
from .instruments import get_hdf_model_path, get_band_wavelength_range, get_instrument_name
...
class AndesSimulator:
    ...
    def __init__(self, config: SimulationConfig):
        ...
        self.simulator = None
        self.sources = None
        self.hdf_path = None  # Track HDF model for header metadata
    ...
    def _update_fits_headers(self, fits_path: Path, hdf_path: Optional[Path] = None) -> None:
        """
        Update FITS file headers with simulation metadata.

        Parameters
        ----------
        fits_path : Path
            Path to the FITS file to update
        hdf_path : Path, optional
            Path to the HDF model used
        """
        with fits.open(fits_path, mode='update') as hdul:
            hdr = hdul[0].header

            hdr['DATE-OBS'] = (datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%S'), 'Observation date')
            hdr['INSTRUME'] = (get_instrument_name(self.config.band), 'Instrument name')
            ...
            if hdf_path is not None:
                hdr['HDFMODEL'] = (hdf_path.name, 'HDF optical model file')

            hdr['SRCTYPE'] = (self.config.source.type, 'Source type')
            hdr['SRCFLUX'] = (self.config.source.flux, 'Source flux')
            hdr['SRCSCALE'] = (self.config.source.scaling_factor, 'Source scaling factor')
            ...
            if self.config.wl_min is not None:
                hdr['WL_MIN'] = (self.config.wl_min, '[nm] Minimum wavelength')
            if self.config.wl_max is not None:
                hdr['WL_MAX'] = (self.config.wl_max, '[nm] Maximum wavelength')

            hdr['HIERARCH E2E_SIM'] = (True, 'Generated by E2E simulator')
    ...
    def _save_line_catalog(self, output_dir: Path) -> None:
        ...
        if sim_type == 'fabry_perot':
            fp = self.source_factory._fp_source
            if fp is None:
                return
            ...
            header_lines = [
                f"band = {fp.band}",
                f"finesse = {fp.finesse}",
                f"gap_mm = {fp.gap_mm:.6f}",
                f"n_refr = {fp.n_refr}",
                f"scaling_factor = {fp.scaling_factor:.2e}",
                f"n_peaks = {len(peaks)}",
                f"unit = nm",
            ]
            wavelengths = peaks
            ...
            lfc = LFCSource(
                ...
                flux_per_line=self.config.source.scaling_factor,
                project_root=self.project_root,
            )
            wls, _ = lfc._generate_lfc_wavelengths(
                wl_min=self.config.wl_min, wl_max=self.config.wl_max)
            header_lines = [
                f"band = {lfc.band}",
        ...
        self.logger.info(f"Line catalog: {catalog_path}")
    ...
    def run_standard_simulation(self, output_path: Optional[Path] = None) -> Any:
        ...
        # Update FITS headers with simulation metadata
        self._update_fits_headers(output_path, self.hdf_path)
        self._save_line_catalog(output_path.parent)
    ...
    def run_single_fiber_batch(self, output_dir: Optional[Path] = None) -> Dict[int, Any]:
        ...
        # Save line catalog once for the whole batch
        if results:
            self._save_line_catalog(output_path.parent)
